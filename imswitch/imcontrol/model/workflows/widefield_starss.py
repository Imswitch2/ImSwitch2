"""WidefieldStarss workflow — acquire polarisation-resolved image stacks.

Ported from WFS WidefieldStarssWorkflow. Acquires H/V polarisation-resolved stacks
...
class WidefieldStarssParams:
    """Parameters for a single WidefieldStarss acquisition sequence.
...
class WidefieldStarssWorkflow:
    """Encapsulates a single H/V WidefieldStarss sequence.
    ...
    Args:
        ...
        params: WidefieldStarss parameters (trigger timings, frame count, etc.).
    ...
    def __init__(
        ...
        # Populated during a WidefieldStarss run
    ...
    def run(self, measurement_name_addition: Optional[str] = None) -> None:
        """Execute a full H/V WidefieldStarss sequence.
        ...
        Args:
            measurement_name_addition: Optional temporary filename suffix for
                this run. If provided, the original params value is restored
                after the sequence.

        Raises:
            ValueError: If both ``record_h`` and ``record_v`` are False.
        """
        previous_name_addition = self.params.measurement_name_addition
        if measurement_name_addition is not None:
            self.params.measurement_name_addition = measurement_name_addition

        try:
            self._run_sequence()
        finally:
            if measurement_name_addition is not None:
                self.params.measurement_name_addition = previous_name_addition

    def _run_sequence(self) -> None:
        """Run the configured WidefieldStarss sequence."""
        ...
        logger.info("WidefieldStarss sequence finished")
