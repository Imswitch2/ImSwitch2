...
def assemble_image(
    ...
    Bilinear splatting: each sample deposits its value into the four
    surrounding raster pixels with bilinear weights; pixels are normalized by
    their accumulated weight. When ``origin``/``shape`` are omitted they are
    derived from the sample extent (origin at the minimum position; one pixel
    per pitch). Pixels whose accumulated weight stays below ``min_weight``
    become NaN — honest holes instead of extrapolated values. Samples outside
    the raster are dropped.
    """
    ...
    if (
        ...
        raise ValueError("positions must be a non-empty (N, 2) array matching values")
    ...
    splat_add(accumulated, weights, positions, values, origin, pitch)
    return finalize_splat(accumulated, weights, origin, pitch, min_weight)
...
def finalize_splat(
    ...
    """Normalize a splat accumulator into a :class:`ScatterAssembly`."""
