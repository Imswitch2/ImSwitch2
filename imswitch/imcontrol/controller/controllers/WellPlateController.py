...
class WellPlateController(ImConWidgetController):
    """ Controller for the WellPlate widget.

    Displays a 96-well plate and exposes positioner move/step API.
    Well-click → stage-move wiring is a TODO; currently clicks are logged only.
    """
    ...
    def attrChanged(self, key, value):
        ...
        if key[3] == _positionAttr:
            self.setPos(positionerName, axis, value)
    ...
    def getPositionerNames(self) -> List[str]:
        """ Returns the device names of all positioners. """
    ...
    def setPositionerStepSize(self, positionerName: str, stepSize: float) -> None:
        """ Sets the step size of the specified positioner to the specified micrometers. """
    ...
    def stepPositionerUp(self, positionerName: str, axis: str) -> None:
        """ Moves the specified positioner axis in positive direction by its step size. """
        self.stepUp(positionerName, axis)

    @APIExport(runOnUIThread=True)
    def stepPositionerDown(self, positionerName: str, axis: str) -> None:
        """ Moves the specified positioner axis in negative direction by its step size. """
