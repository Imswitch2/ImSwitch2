...
def test_tiff_snap_without_attrs_carries_only_the_recording_bookkeeping(detman, tmp_path):
    """Every snap's OME-XML is rewritten so the recording bookkeeping (frame
    counts, outcome) and any acquisition layout reach the file; with no
    shared attrs there is no scan-axis provenance to carry."""
    ...
    with tifffile.TiffFile(str(tmp_path / 'plain_Cam.ome.tiff')) as t:
        xml = t.ome_metadata or ''
        assert 'ScanStage:' not in xml
        assert 'recording:actual_frames' in xml
