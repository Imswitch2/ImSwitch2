...
# Bind to THIS checkout's ImSwitch: Python puts the script's own directory on
# sys.path, never the working directory, so without this the script silently
# exercised whichever ImSwitch is installed -- another branch's designer, on a
# machine with several checkouts -- and reported its crashes as this one's.
_REPO_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
if _REPO_ROOT not in sys.path:
    sys.path.insert(0, _REPO_ROOT)
import imswitch  # noqa: E402
print(f'interpreted by: {os.path.dirname(imswitch.__file__)}')
...
def run(name, target, lengths, steps, use_beta=False, centers=None):
    ...
    if use_beta:
        p['return_time'] = 0.01
        # A stage dwell: the Beta designer refuses a dwell that is not longer
        # than the stage's move + settle (4 ms by default), and 20 us is the
        # galvo point-scan default, not a stage's.
        p['sequence_time'] = 0.01
