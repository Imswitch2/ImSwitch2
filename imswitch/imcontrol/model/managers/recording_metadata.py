...
def _add_map_annotation(xml: str, annotations: Dict[str, Any]) -> str:
    ...
    image fields only. Shared recording attributes (e.g. the write-only
    ``ScanStage:scan_axis_devices``/``scan_axis_physical`` provenance) are
    arbitrary key/value metadata, for which OME defines ``MapAnnotation``.
    """
