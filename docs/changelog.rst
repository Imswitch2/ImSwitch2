...
- Tiling gained an **Advanced alignment** option that brings the offline reconstructor's global solve to acquisition. The plain pass correlates each tile once, against the canvas region its commanded position points at, and applies whatever comes back — so a false match places that tile and every tile placed relative to it afterwards, and that layout is what gets saved. Advanced alignment correlates against *every* already-placed neighbour the tile overlaps (a spiral usually offers two to four), places it at the confidence-weighted consensus, and once the run ends re-solves the whole layout by weighted least squares with inconsistent measurements rejected against the consensus. The solved layout is what reaches the overview, the saved mosaic and the sidecars. On simulated 25–49 tile spirals it removed the placement error entirely on textured samples (3.6 px RMS / 23 px worst → 0.0) and helped less on very sparse ones, where there is often only one usable neighbour anyway. It is also **faster**, which is why it is the default rather than an opt-in: the plain pass rebuilds and copies the whole growing mosaic to correlate against once per tile, where this correlates against the two to four small tiles that actually border the new one. Measured on 100-tile spirals at 273–1024 px tiles, it ran in 0.41–0.49x the time of the pass it replaces. Untick **Advanced alignment**, or set ``tiling.advancedAlignment: false``, to reproduce the older single-neighbour behaviour.
