...
**Status:** Reference — audit of `codex/acquisition-layout-schema` at `8fb56d6a`; conditions 1–7 done 2026-09-04 — cleared for rig validation ([rig guide](../../acquisition-layout-rig-validation.md))
**Date:** 2026-09-03
...
7. Controller-level seam tests: real `getAcquisitionLayouts` per family -> `startRecording` -> real `RecordingWorker` -> storer -> DataObj/resolver, including one scan-driven detector (assert `DETECTOR_PAYLOAD_MISMATCH`/`EMPTY_FRAME_SELECTION` fire) and one single-file lapse `one-group-per-item` round trip. **Done 2026-09-04:** `imswitch/imcontrol/_test/unit/test_acquisition_layout_recording_seam.py` runs three recordings through every stage with nothing faked at the seams — the real Galvo designer and Advanced TTL designer make the scan, the producer builders make the layouts (the controller-level accessor for the camera family, with real `scan_devices`/`scan_directions`/`physical_kind_overrides`), `startRecording` gates them, the real recording worker and HDF5 storer write the file under the simulated NI-DAQ with the repository's own mock camera and APD managers, and the ImProcess resolver reads the layout back byte-identical and authoritative. Families: a camera point scan (frame stream, `validate_detector_edge_counts` against the TTL that actually ran, `recording:discarded_frames == 0`); two APDs on a two-line-step scan (assembled payload with a `condition` storage axis, `(1, 2, Y, X)` on disk) plus the gate refusing a frame-stream layout for an assembling detector with `DETECTOR_PAYLOAD_MISMATCH` before any writer opens; and a two-item single-file scan lapse on disk, `scan0`/`scan1` groups in one HDF5 file, each resolving to its own `time` partition (`one-group-per-item`) and, partition aside, to the same layout. `EMPTY_FRAME_SELECTION` is not reachable through a producer: `RecordedEventSpan` validation refuses a zero count and the Advanced builder refuses a detector with no recorded pulse, so the gate's check is a belt for hand-written layouts only. Test-infrastructure finding on the way: `_ensure_qcore_app()` in the coordinator tests drops its only reference to the `QCoreApplication`, and under PyQt5 that tears down every Python-owned QObject — managers included — at the next garbage collection; single-session tests never noticed, the lapse test's second session found both managers gone. The new file holds the application in a fixture; the coordinator tests are unchanged and remain correct only by accident of ordering.
